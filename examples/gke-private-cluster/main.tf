...
//  default = "gce-uefi-images/cos-stable"
}
resource "google_compute_instance" "bastion_host" {
  ...
  metadata_startup_script = <<EOF
    local readonly version=$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)
    curl -LO "https://storage.googleapis.com/kubernetes-release/release/$version/bin/linux/amd64/kubectl"

    echo "gcloud container clusters get-credentials ${var.cluster_name} --region ${var.region} --project ${var.project}" >> /etc/profile
EOF
...
//          cidr_block   = "0.0.0.0/0"
          cidr_block   = "${google_compute_instance.bastion_host.network_interface[0].network_ip}/32"
